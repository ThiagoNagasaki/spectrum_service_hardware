...
/**
 * \brief Função auxiliar para mapear enum BaudRate -> speed_t do POSIX.
 */
...
public:
        ...
        , status_(TransportStatus::Disconnected)
    {
        Logger::instance().debug(CommandContext::HARDWARE,
                                 fmt::format("RS232Transport Impl criado para dispositivo {} (baud enum={})",
    ...
    bool connect() {
        ...
        if (fd_ < 0) {
            Logger::instance().error(CommandContext::HARDWARE, ErrorCode::RS232ConnectionFailure,
        ...
        if (::tcgetattr(fd_, &tty) != 0) {
            Logger::instance().error(CommandContext::HARDWARE, ErrorCode::RS232ConfigurationFailure,
                                     fmt::format("Erro ao obter atributos da porta RS232: {}", device_));
            status_ = TransportStatus::Error;
            return false;
        }

        // Converte enum -> speed_t
        ...
        Logger::instance().info(CommandContext::HARDWARE,
                                fmt::format("Conectado à porta RS232: {} (baud {})",
    ...
    bool send(const std::vector<uint8_t>& data) {
        if (status_ != TransportStatus::Connected || fd_ < 0) {
            Logger::instance().warning(CommandContext::HARDWARE,ErrorCode::RS232DataSendFailure,
        ...
        if (bytes_written < 0 || static_cast<size_t>(bytes_written) != data.size()) {
            Logger::instance().error(CommandContext::HARDWARE, ErrorCode::RS232DataSendFailure,
    ...
    void subscribe(std::function<void(const std::vector<uint8_t>&)> callback) {
        ...
        // sem leitura assíncrona, esse callback não é acionado sozinho
    }
