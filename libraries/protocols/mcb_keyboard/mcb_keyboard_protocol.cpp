...
public:
  ...
  std::vector<uint8_t> buildFrame(MCBCommand cmd,
  ...
  std::optional<MCBFrame> parseFrame(const std::vector<uint8_t> &buffer) const {
    ...
    if (buffer.size() < MCB_MIN_FRAME_SIZE || buffer.front() != STX ||
        buffer.back() != ETX) {
    ...
    if (buffer.size() != expected) {
      Logger::instance().warning(
          ...
          fmt::format(
              "MCBProtocol: tamanho inconsistente, esperado={}, real={}",
    ...
    if (dataSize) {
      data.insert(data.end(), buffer.begin() + 3,
    ...
    if (recvChk != calcChk) {
      Logger::instance().warning(
          CommandContext::HARDWARE, ErrorCode::ProtocolError,
          fmt::format("Checksum inválido: calc=0x{:02X}, recv=0x{:02X}",
                      calcChk, recvChk));
      return std::nullopt;
    }
...
private:
  ...
  static MCBCommand toMCBCommand(uint8_t b) {
    ...
    switch (b) {
    case 0x51:
      return C::READ_FIRMWARE;
    case 0x52:
      return C::READ_STATUS;
    case 0x54:
      return C::READ_ANALOG_INPUT;
    case 0x5A:
      return C::READ_CHAVE_LIGA;
    case 0x5B:
      return C::READ_DATA_HORA;
    case 0x63:
      return C::WRITE_DIGITAL_OUT;
    default:
      return C::UNKNOWN;
    }
  }

  static uint8_t calculateChecksum(MCBCommand cmd,
...
MCBProtocol::MCBProtocol(
    ...
    : pImpl_(
          std::make_unique<Impl>(std::move(transport), std::move(receiver))) {}

MCBProtocol::~MCBProtocol() = default;

std::vector<uint8_t>
MCBProtocol::buildFrame(uint8_t cmd, const std::vector<uint8_t> &payload) {
  return pImpl_->buildFrame(static_cast<MCBCommand>(cmd), payload);
}

std::optional<std::vector<uint8_t>>
MCBProtocol::parseFrame(const std::vector<uint8_t> &buffer) {
